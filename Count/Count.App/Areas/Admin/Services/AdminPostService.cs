...
namespace Count.App.Areas.Admin.Services
{
    public class AdminPostService:IAdminPostService
    {
        ...
        public AdminPostService(CountDbContext dbContext)
        {
        ...
        public async Task DeletePost(DeletePostBindingModel model)
        {
            var post = await FindPost(model.Id);
            post.IsDelete = true;
        ...
        public async Task<Post> FindPost(int id)
        {
            ...
            if (post == null)
            {
                throw new NullReferenceException($"No post with id: {id}.");
            }
            return post; 
        }
